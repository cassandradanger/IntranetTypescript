...
export default class BDayAnniversaryWebPart extends BaseClientSideWebPart<IBDayAnniversaryWebPartProps> {
  ...
  private _renderList(items: ISPList[], startDay, startMonth, endDay, endMonth, currentYear, numberOfMonths): void {  
    ...
    items.forEach((item: ISPList) => {
      ...
      var bdayDate = ''
      if(numberOfMonths === 1){
        if(item.Birth_x0020_Day >= startDay && item.Birth_x0020_Day <= endDay && item.Birth_x0020_Month === startMonth){
          occasion = 'Birthday'
          bdayDate = item.Employee_x0020_Birthday
        } else {
          occasion = 'Anniversary'
          years = currentYear - item.AnniversaryYear
        }
      } else {
        console.log('2 months goes here')
      }
      html += `  
        <li>
          ...
          <div class=${styles.personWrapper}>
            ...
            <p class=${styles.reason}>${bdayDate}</p>
            <p class=${styles.reason}>${years} year(s)</p>
          </div>
        </li>
        `;  
    });  
